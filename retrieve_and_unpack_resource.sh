...
if $retrieve_generic; then
    ...
    python3 "$_main_repo/utils/downloads.py" retrieve -i "$_root_dir/extras.ini" -c "$_download_cache"
    python3 "$_main_repo/utils/downloads.py" unpack -i "$_root_dir/downloads.ini" -c "$_download_cache" "$_src_dir"
    python3 "$_main_repo/utils/downloads.py" unpack -i "$_root_dir/extras.ini" -c "$_download_cache" "$_src_dir"
fi
